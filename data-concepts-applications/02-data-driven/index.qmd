...
## Features of data-driven decision-making {.smaller}
...
#| fig-cap: "Participants' responses to the question: What is data?"
#| fig-align: center
#| echo: false

data_definitions <- read.csv(
  file = here::here("data", "definitions_2026.csv")
)

data_definitions |>
  dplyr::mutate(
    themes = stringr::str_split(string = themes, pattern = "; ")
  ) |>
  tidyr::unnest(themes) |>
  dplyr::pull(themes) |>
  table() |>
  ...
  stats::setNames(c("data_definition", "n")) |>
  ggplot2::ggplot(ggplot2::aes(x = n, y = reorder(data_definition, n))) +
  ggplot2::geom_col(fill = oxthema::get_oxford_colour("orange"), alpha = 0.8) +
  ggplot2::geom_text(
    ggplot2::aes(label = n),
    hjust = -1,
    size = 3
...
```{r}
#| label: data-definition-primary
#| fig-cap: "Primary data definitions inferred from participants' responses to the question: What is data?"
#| fig-align: center
#| echo: false

data_definitions |>
  dplyr::count(primary_theme) |>
  dplyr::arrange(desc(n)) |>
  ggplot2::ggplot(ggplot2::aes(x = n, y = reorder(primary_theme, n))) +
  ggplot2::geom_col(fill = oxthema::get_oxford_colour("ochre"), alpha = 0.8) +
  ggplot2::geom_text(
    ggplot2::aes(label = n),
    hjust = -1,
    size = 3
...
```{r}
#| label: data-roles-any
#| fig-cap: "Various data roles reported by participants"
#| fig-align: center
#| echo: false

data_roles <- read.csv(
  file = here::here("data", "data_roles_2026.csv")
)
...
data_roles |>
  ...
  as.data.frame() |>
  dplyr::arrange(desc(Freq)) |>
  stats::setNames(c("data_role", "n")) |>
  ggplot2::ggplot(ggplot2::aes(x = n, y = reorder(data_role, n))) +
  ggplot2::geom_col(fill = oxthema::get_oxford_colour("mauve"), alpha = 0.8) +
  ggplot2::geom_text(
    ggplot2::aes(label = n),
    hjust = -1,
    size = 3
...
```{r}
#| label: data-roles-primary
#| fig-cap: "Primary data role inferred from participants' reported data roles"
#| fig-align: center
#| echo: false
...
data_roles |>
  ...
  dplyr::arrange(desc(n)) |>
  ggplot2::ggplot(ggplot2::aes(x = n, y = reorder(primary_theme, n))) +
  ggplot2::geom_col(fill = oxthema::get_oxford_colour("ocean"), alpha = 0.8) +
  ggplot2::geom_text(
    ggplot2::aes(label = n),
    hjust = -1,
    size = 3
